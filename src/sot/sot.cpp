...
Sot::Sot(const std::string& name)
  ...
  docstring =
      ...
      "    \n"
      "      Input:\n"
      "        - a positive integer : number of degrees of freedom of the "
      "robot.\n"
      "    \n";
...
void Sot::clear(void) {
  for (std::list<TaskAbstract*>::iterator it = stack.begin(); stack.end() != it;
       ++it) {
    removeDependency(**it);
  }
  stack.clear();
...
dynamicgraph::Matrix& Sot::computeJacobianConstrained(
    const dynamicgraph::Matrix& Jac, const dynamicgraph::Matrix& K,
...
dynamicgraph::Matrix& Sot::computeJacobianConstrained(
    const TaskAbstract& task, const dynamicgraph::Matrix& K) {
...
dynamicgraph::Matrix& Sot::computeConstraintProjector(
    dynamicgraph::Matrix& ProjK, const int& time) {
  sotDEBUGIN(15);
