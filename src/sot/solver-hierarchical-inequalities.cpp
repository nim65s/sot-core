...
void bubRemoveColumn(bubTemplateMatrix& M, const unsigned int col) {
  for (unsigned int j = col; j < M.size2() - 1; ++j)
    for (unsigned int i = 0; i < M.size1(); ++i) M(i, j) = M(i, j + 1);
  for (unsigned int i = 0; i < M.size1(); ++i) M(i, M.size2() - 1) = 0;
}
void bubRemoveColumn(bub::triangular_matrix<double, bub::upper>& M,
                     const unsigned int col) {
  for (unsigned int j = col; j < M.size2() - 1; ++j)
    for (unsigned int i = 0; i <= j; ++i) M(i, j) = M(i, j + 1);
  for (unsigned int i = 0; i < M.size1(); ++i) M(i, M.size2() - 1) = 0;
}
template <typename bubTemplateMatrix>
...
ConstraintMem::ConstraintMem(const ConstraintMem& clone)
    : active(clone.active),
      equality(clone.equality),
      notToBeConsidered(false),
      Ji(0),
      eiInf(clone.eiInf),
      eiSup(clone.eiSup),
      boundSide(clone.boundSide),
      activeSide(clone.activeSide),
      rankIncreaser(clone.rankIncreaser),
      constraintRow(clone.constraintRow),
      range(clone.range),
      lagrangian(clone.lagrangian),
      Ju(clone.Ju),
      Jdu(clone.Jdu) {
  if (clone.Ji.size()) {
...
template <typename bubTemplateMatrix>
unsigned int SolverHierarchicalInequalities::rankDetermination(
    const bubTemplateMatrix& A, const double threshold) {
  unsigned int res = 0;
...
void SolverHierarchicalInequalities::initializeDecompositionSlack(void) {
  ...
  for (unsigned int i = 0; i < ranks; ++i) orderS[i] = i;
