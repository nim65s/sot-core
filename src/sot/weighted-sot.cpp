...
WeightedSot::WeightedSot(const std::string& name)
    : Sot(name),
      ...
      constrainedWeightSIN(
          NULL, "sotWeightedSOT(" + name + ")::input(matrix)::Kweight"),
...
dynamicgraph::Matrix& WeightedSot::computeSquareRootInvWeight(
    dynamicgraph::Matrix& S5i, const int& time) {
  sotDEBUGIN(15);
  ...
  for (unsigned int i = 0; i < A.cols(); ++i) {
    double x = A(i, i);
    for (unsigned int k = 0; k < i; ++k) x -= S5(k, i) * S5(k, i);
    double sq = sqrt(x);
    S5(i, i) = sq;

    for (unsigned int j = i + 1; j < A.cols(); ++j) {
      x = A(i, j);
      for (unsigned int k = 0; k < i; ++k) x -= S5(k, i) * S5(k, j);
      S5(i, j) = x / sq;
    }
  }
...
dynamicgraph::Matrix& WeightedSot::computeConstrainedWeight(
    dynamicgraph::Matrix& KAK, const int& iterTime) {
  sotDEBUGIN(15);
  ...
  KAK.fill(0);
  {
    ...
    for (unsigned int i = 0; i < mJ; ++i)
      for (unsigned int j = 0; j < 6; ++j)
        for (unsigned int k = 0; k < 6; ++k) KA(i, j) += K(k, i) * A(k, j);
...
dynamicgraph::Vector& WeightedSot::computeWeightedControlLaw(
    dynamicgraph::Vector& control, const int& iterTime) {
  sotDEBUGIN(15);
