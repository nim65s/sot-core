...
class sotHouseholdMatrices {
 ...
 public:
   ...
   * double product. */
  virtual void multiplyRangeRight(
      const Vector& a,  // N*A = Q*[0;I]*A = P1*...*Pn*[0;A]
...
dynamicgraph::Matrix& SotQr::computeConstraintProjector(
  ...
  for (unsigned int i = 0; i < J.rows(); ++i) {
    if (ffJointIdFirst)
      for (unsigned int j = 0; j < ffJointIdFirst; ++j) Jc(i, j) = J(i, j);
